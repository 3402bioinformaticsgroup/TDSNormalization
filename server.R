...
output$plotProcessedCV<-renderPlot({
    ...
    labels<-matrix(rep(0, dim(processedReadSummary)[1]*4), ncol=4)
    rownames(labels)<-rownames(processedReadSummary)
    colnames(labels)<-c("name","color","x","y")
    for (i in 1:length(CodeClass)){
        if(grepl("Housekeeping", CodeClass[i])){
            if((processedReadSummary[i,2]/processedReadSummary[i,1])>0.3){
                labels[i,1]<-rownames(labels)[i]
                labels[i,2]<-"red"
                labels[i,3]<-i
                labels[i,4]<-processedReadSummary[i,2]/processedReadSummary[i,1]
            }
        }
    }
    text(as.numeric(labels[,3])-20, as.numeric(labels[,4])*100, labels=labels[,1], col=labels[,2], cex=0.5)
